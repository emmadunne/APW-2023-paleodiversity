...
## Like above, we need to get the genus incidence frequencies for each interval,
##    but this time we don't need the totals at the beginning of the string
freq_data2 <- lapply(1:nrow(intervals), function(i) {
  tmp <- genus_data %>% filter(max_ma >= intervals[i,"max_ma"] & min_ma <= intervals[i,"min_ma"]) %>% 
    count(., genus) %>% arrange(desc(n)) %>% 
    select(n)
  freq_raw <- as.numeric(tmp$n)
...
names(freq_data2) <- intervals$interval_name # give each list element its correct interval name
str(freq_data2) # call up the list to see a summary of its structure
freq_data2[[6]] # check the data for the Norian


## Using these data, you can plot rank abundances for each interval
## Let's look at the Norian:
barplot(freq_data2[["Norian"]], log = "y", # log the y-axis
        col = "hotpink", border = 0, space = 0,
...
sauares_plot <- ggplot(to_plot, aes(x = mid_ma, y = squares_list)) +
  geom_line(colour = "hotpink", size = 1) + 
  geom_point(colour = "hotpink", size = 4, shape = 16) +
  scale_x_reverse(breaks = int_boundaries) + 
  labs(x = "Time (Ma)", y = "Squares genus diversity") +
  theme_minimal()
sauares_plot # call to plot tab
